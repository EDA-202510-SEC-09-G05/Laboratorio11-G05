...
def _dfs_vertex_recursive_helper(search, graph, vertex):
    ...
    current_adj_list_for_debug = []
    if adj_vertex_keys_list:
        for i in range(lt.size(adj_vertex_keys_list)):
            current_adj_list_for_debug.append(lt.get_element(adj_vertex_keys_list, i))
